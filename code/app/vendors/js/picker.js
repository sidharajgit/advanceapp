...
(function(){
    ...
    var CalenderCtrl = function($scope, $timeout, picker, $mdMedia){
        ...
        var x = new Date();
        self.maxDate = (x.getMonth()+1)+"-"+(x.getDate()+1)+"-"+x.getFullYear();//$scope.maxDate;			//Maximum date
		//console.log(self.maxDate," - cure date  ",self.picker.dateCont);			//Maximum date
...
function picker(){
    ...
    var rangeDefaultListCopy = rangeDefaultList;
    var dateCont = {'startDate': moment().subtract(6, 'd'),'endDate': moment()};
    ...
    return {
        ...
        $get: function(){
            return {
                ...
                rangeDefaultListCopy:rangeDefaultListCopy,
                format : format,
                dateCont : dateCont
            }
        }
...
(function(){
    ...
    function RangePickerInput($document,$mdMedia,$mdUtil,picker){
        return {
            ...
            link: function(scope, $element, attr, ctrl){
                if(scope.vm.customview){
                    //if(scope.vm.customview) picker.rangeDefaultList = [];
                    //else picker.rangeDefaultList = picker.rangeDefaultListCopy;
                    RangePickerCtrl.prototype.showCustomView();
                }
                ctrl[0].$render = function() {
                    scope.vm.value = this.$viewValue;
                }
                scope.$watch('vm.value', function() {
                    if(typeof scope.vm.value === 'object'){
                        scope.vm.changeDate({data:scope.vm.value})
                    };
                });
            }
        }
...
var RangePickerCtrl = function($scope, picker){
    ...
    self.startDate = moment(picker.dateCont.startDate, self.format);
    self.endDate = moment( picker.dateCont.endDate, self.format);
...
RangePickerCtrl.prototype.setNgModelValue = function(startDate, divider, endDate) {
  ...
  var range = {startDate: startDate, endDate: endDate, startDateAsMoment: momentStartDate, endDateAsMoment: momentEndDate};
    ...
    self.selectedTabIndex = 0;
    self.view ='DATE';
    self.startDate = moment(startDate, self.format );
    self.endDate = moment(endDate, self.format);
...
RangePickerCtrl.prototype.cancel = function() {
    ...
    if(self.customToHome && self.showCustom) {
        ...
        self.selectedTabIndex =0;
        self.showCustom=false;
        self.startDate = moment(self.pickerNew.dateCont.startDate, self.format);
        self.endDate = moment( self.pickerNew.dateCont.endDate, self.format);
        self.scope.$emit('range-picker:close');
    }
}
